      ...
      <section className="relative h-[85vh] min-h-[600px] flex items-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-container-max mx-auto w-full px-margin-mobile md:px-margin-desktop">
          <div className="max-w-xl">
            <span className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm text-white/90 text-xs font-semibold px-4 py-2 rounded-full mb-6 border border-white/10">
              <span className="w-2 h-2 bg-[#F97316] rounded-full animate-pulse"></span>
              Trusted by 500+ Businesses Across Kerala
            </span>
            ...
            <div className="flex flex-wrap gap-4">
              <Link href="/products">
                <Button className="bg-[#F97316] text-white font-semibold text-sm px-8 py-5 rounded-lg hover:bg-orange-600 transition-all shadow-lg shadow-orange-500/20 hover:shadow-orange-500/30 hover:-translate-y-0.5">
                  Explore Products
                </Button>
              </Link>
              <Link href="/contact">
                <Button className="bg-white/10 backdrop-blur-sm text-white border border-white/20 font-semibold text-sm px-8 py-5 rounded-lg hover:bg-white/20 transition-all">
                  Request Quote
                </Button>
              </Link>
            </div>
        ...
        <div className="max-w-container-max mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {[
            ...
            <div key={stat.label} className="flex flex-col items-center py-4">
              <span className="font-heading text-4xl md:text-5xl font-extrabold text-primary mb-2">{stat.value}</span>
              <span className="text-xs text-on-surface-variant uppercase tracking-[0.12em] font-semibold">{stat.label}</span>
            </div>
          ))}
        </div>
      </section>
            ...
            {categories.map((cat) => (
              <Link key={cat.id} href="/products">
                <div className="bg-white rounded-2xl p-6 text-center shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer border border-outline-variant/20 hover:border-primary/30 group">
                  <div className="w-16 h-16 bg-primary/5 text-primary rounded-2xl flex items-center justify-center mx-auto mb-4 group-hover:bg-primary group-hover:text-white transition-all duration-300 group-hover:rotate-3">
      ...
      <section className="py-section-padding bg-primary text-white text-center relative overflow-hidden">
        ...
        <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop relative z-10">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Link href="/contact">
              <Button className="bg-[#F97316] text-white font-semibold px-8 py-4 rounded-lg hover:bg-orange-600 transition-all shadow-lg shadow-orange-500/20 hover:-translate-y-0.5">
                Request Free Consultation
              </Button>
            </Link>
            <Link href="/kitchen-solution-builder">
              <Button variant="outline" className="border-white/30 text-white hover:bg-white/10 font-semibold px-8 py-4 rounded-lg backdrop-blur-sm">
